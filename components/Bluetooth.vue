<template>
  <div class="flex items-center space-x-4">
    <div class="flex flex-row gap-2">
      <img v-if="status == 'Disconnected'" src="~/assets/StatusSymbolRed.svg" />
      <img v-if="status == 'Connected'" src="~/assets/StatusSymbolGreen.svg" />
      <img v-if="status == 'Error'" src="~/assets/StatusSymbolOrange.svg" />
      <p>{{ status }}</p>
    </div>
    <button
      class="bg-blue-700 px-6 py-1 rounded-lg text-white uppercase"
      @click="connect"
    >
      Connect
    </button>
    <button
      class="bg-green-700 px-6 py-1 rounded-lg text-white uppercase"
      @click="test"
    >
      Refresh
    </button>
    <button
      class="bg-blue-700 px-6 py-1 rounded-lg text-white uppercase hidden"
      @click="ping"
    >
      Ping
    </button>
    <button
      class="bg-gray-300 px-6 py-1 rounded-lg text-black uppercase"
      v-if="bleCharacteristic"
      @click="calibrate"
    >
      Calibrate
    </button>
    ...
    const writeToCharacteristic = async (
      ...
      try {
        ...
        if (encoded.length > 20) {
          throw new Error(
            `Data too long: ${encoded.length} bytes (max 20 bytes)`
          );
        }
          ...
          (event: any) => {
            try {
              ...
              if (text.includes("\n")) {
                ...
                const processedData = receivedChunks
                  .map((chunk) => {
                    ...
                    return {
                      user_id: 1,
                      date: formattedDate,
                      weight: (Number(value.trim()) / 100) * bottle ,
                    };
                  })
                ...
                const { data } = useFetch(
                  "https://192.168.123.16/api/sensor-data",
                  {
                    method: "POST",
                    headers: {
                      "Content-Type": "application/json",
